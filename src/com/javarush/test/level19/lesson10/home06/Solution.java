...
public class Solution {
    ...
    public static void main(String[] args) throws IOException
    {
        ...
        for (int i = 0; i < stringList.size(); i++) {
            //проверяем, содержатся ли в строке цифры 1-12, чтобы лишний раз не гонять проверку
            if (stringList.get(i).matches(".*\\b((1[012])|[0-9])\\b.*")){
                String[]strs = stringList.get(i).split(" ");
                String newLine = "";
                for (String word : strs){
                    if (word.matches("((1[012])|[0-9])")) { //проверяем, является ли слово цифрами 1-12
